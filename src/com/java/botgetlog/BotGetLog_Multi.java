...
//  For runtime measurement
    ...
    private static void rebuildExcelCache(Workbook workbook) {
        ...
        try {
            Sheet deviceSheet = workbook.getSheet("deviceList");
            if (deviceSheet != null) {
                for (Row row : deviceSheet) {
                    ...
                    DEVICE_ROW_CACHE.put(cacheRowNum,
                            new CachedDeviceRow(getCell(row, 0), getCell(row, 2), getCell(row, 3)));
                }
            }
        } catch (Exception e) {
    ...
    private static Map<String, String> readSettingValues(Sheet sheet) {
        ...
        for (Row row : sheet) {
            ...
            values.put(key, safeValue(getCell(row, 1)));
        }
        return values;
    }
    ...
    private static String buildGatewayServerValue(String hostValue, String portValue) {
        ...
        if (!normalizedPort.isEmpty()) {
            if (!normalizedPort.matches("\\d+")) {
                throw new IllegalArgumentException("Invalid 'gatewayPort' in _setting sheet. Use 22 for SSH or 23 for TELNET.");
            }
            ...
            throw new IllegalArgumentException("Unsupported 'gatewayPort' in _setting sheet. Use 22 for SSH or 23 for TELNET.");
        }
    ...
    private static CachedSettings loadSettingsFromSheet(Sheet sheet) {
        if (sheet == null) {
            throw new IllegalArgumentException("Missing '_setting' sheet.");
        }
        ...
        if (server.isEmpty()) {
            throw new IllegalArgumentException("Missing 'gatewayHost' in _setting sheet.");
        }
    ...
    private static CredentialInput showCllsCredentialDialog(String initialUsername, String initialPassword) {
        JTextField usernameField = new JTextField(safeValue(initialUsername), 20);
        JPasswordField passwordField = new JPasswordField(safeValue(initialPassword), 20);
        ...
        panel.add(new JLabel("<html>Please enter your CLLS login credentials.<br>(Make sure your keyboard is set to English before typing.)</html>"));
        ...
        new Thread(() -> {
            ...
            try {
                ...
                for (int i = 0; i < workbook.getNumberOfSheets(); i++) {
                    ...
                    if ("_setting".equals(sheet.getSheetName())) {
...
//  CLLS credentials
                        if (User_CLLS == null || User_CLLS.trim().isEmpty()
                            ...
                            Sheet deviceSheet = workbook.getSheet("deviceList");
                            List<ValidationTarget> validationTargets = collectValidationTargets(deviceSheet);
                            ...
                            if (enteredCredentials == null) {
                                D.Error("CLLS login was cancelled. Please restart the program when ready.");
                                return;
                            }
                                        ...
                                        try {
                                            ...
                                            if (exitCode == 0) {
                                                ...
                                                if (networkLost.get()) {
                                                    ...
                                                    synchronized (BotGetLog_Multi.class) {
                                                        networkLost.set(false);
                                                        BotGetLog_Multi.class.notifyAll();
                                                    }
                                                } else {
...
//   ()  Ping 
                    ...
                    } else if ("deviceList".equals(sheet.getSheetName())) {
                        List<Integer> indexRowList = new ArrayList<>();
                        for (Row row : sheet) {
                            try {
                                if ("Y".equals(getCell(row, 0))
...
//   ThreadPool - TELNET_LIMIT - Telnet_Multi
                        ...
                        BotGetLog_Multi.attachExecutor(exec);
...
//  Real-time mode/thread/CPU monitor
                        ...
                        Thread statusThread = new Thread(() -> {
                            ...
                            while (!exec.isShutdown()) {
                                try {
                                    ...
                                    int total = BotGetLog_Multi.isFocusModeActive() ? cpuMax : Telnet_Multi.NORMAL_TELNET_LIMIT;
                                    int queue = exec.getQueue().size();
                                    String mode = BotGetLog_Multi.isFocusModeActive() ? "TURBO" : "NORMAL";
                            ...
                            for (NodeCommandTask task : batchTasks) {
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        BotGetLog_Multi.successCount.incrementAndGet();
    ...
    public static synchronized void setFocusMode(boolean mode) {
        ...
        ThreadPoolExecutor execNow = BotGetLog_Multi.getExecutor();
        if (execNow == null) {
...
//   Summary  ( % )   Unique + --
    ...
    private static void showSummary(PathFile FileInput, int totalProcessed) {
        try {
            ...
            //  Success = ---
            int successCount = BotGetLog_Multi.successCount.get();
...
//  log static  static method
    ...
    private static File findLatestCompletedLog(PathFile fileInput, int rowNum, String loopback, String device, String cmdSet, String lastCommand) {
        try {
            ...
            Arrays.sort(matched, BotGetLog_Multi::compareLogPriority);
        ...
        try {
            ...
            if (cachedRow == null || !settings.isConfigured() || lastCommand.isEmpty()) {
                ...
                try ( Workbook workbook = WorkbookFactory.create(excelFile)) {
                    ...
                    Sheet setSheet = workbook.getSheet("_setting");
                    if (setSheet != null) {
