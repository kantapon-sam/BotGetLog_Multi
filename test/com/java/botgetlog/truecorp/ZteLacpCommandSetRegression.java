...
public final class ZteLacpCommandSetRegression {
    private static final String SET="ZTE-LLDP-Link_OPTIC", COMMAND="show lacp internal";
    public static void main(String[] args) throws Exception {
        ...
        check(changed.equals(new TreeSet<String>(Arrays.asList("cmdSet:7:5","cmdSet:8:5"))),"Unexpected workbook changes: "+changed);
        List<String> commands=commands(current,SET);
        check(commands.equals(Arrays.asList("terminal length 0","show processor","show lldp neighbor brief","show version","show opt brief","show interface",COMMAND,"quit")),"Wrong ZTE command order");
        check(!commands(current,"HW-LLDP-Link_OPTIC").contains(COMMAND),"ZTE command leaked into Huawei");
        check(!commands(current,"N-LLDP-Link_OPTIC").contains(COMMAND),"ZTE command leaked into Nokia");
        ...
        check(commands(user,SET).equals(commands),"Existing user did not receive ZTE command");
        ...
        System.out.println("ZTE_LACP_COMMAND_REGRESSION_OK: command order, vendor isolation, unrelated cells, existing-user sync, user data preservation, idempotence, backup");
    }
